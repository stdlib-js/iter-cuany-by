...
tape( 'the function returns an iterator which cumulatively tests whether at least one iterated value passes a test implemented by a predicate function', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = iterCuAnyBy( array2iterator( values ), predicate );
...
tape( 'the returned iterator has a `return` method for closing an iterator (no argument)', function test( t ) {
	...
	it = iterCuAnyBy( randu(), predicate );
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	it1 = iterCuAnyBy( rand, predicate );
...
tape( 'the function supports specifying the predicate function execution context', function test( t ) {
	...
	it = iterCuAnyBy( array2iterator( values ), predicate, ctx );
